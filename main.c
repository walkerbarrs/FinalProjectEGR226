 ...
 * to set the time and alarm using pushbuttons, allows the user to set time and alarm as well as                      read time and alarm from an external serial, and has a wakeup state that occurs 5 minutes before the alarm that slowly turns on LEDs
 */

float voltage=0;
uint8_t alarmChange = 1;    //variable to tell the program if the temporary time need updating
uint8_t alarmFlag = 0;      //flag to allow the program to toggle the speaker on and off
uint8_t alarmSound = 0; //flag to tell the program it is time to sound the alarm
uint8_t speaker = 0;        //variable to turn the speaker on and off
uint8_t lightcount = 0;     //variable to hold the count of the light 0 - 100 for the intensity
...
uint8_t currentState = 0;   //this will tell the handlers when to act and when to not so that weird things aren’t updated when they are not supposed to
...
void T32_INT1_IRQHandler(void)
{
    //set the global flag true and then clear timer 32 interrupt flag
    counting = 1;
    TIMER32_1->INTCLR = 0;
}
...
void updateClock()
{
        seconds ++;             //every time this function is called add one to seconds
...
void PORT3_IRQHandler(void)
{
    ...
    else if(P3->IFG & BIT3)
    {
        ...
        else  snoozedec = 1;                  //set decrement to 1
...
void checkClockChange()
{
    if((clockChange %2) == 0)
        {
            TIMER32_1->LOAD = 50000;         //Load .0167 seconds into the timer = 50000
        }
    else
    {
...
void blinkCLKMinutes()
{
    ...
    ms_delay(1000);
    if (blinking) blinking = 0; //toggle blinking every time the function is called
    else blinking = 1;
}
...
void blinkALRMMinutes()
{
    ...
    ms_delay(1000);
    if (blinking) blinking = 0; //toggle blinking every time this funciton is called
    else blinking = 1;
}
...
//custom function to check the states of various things when alarmset is 1 with no outputs
void alarmSetConditions()
{
    ...
    {
        if(((hours == tempHours)||(hours==Ahours)) && (((tempMinutes - minutes) <= 5)||((Aminutes - minutes) <= 5)) && (((am_pm % 2) == (tempAmpm%2))||((am_pm%2) == ((Aam_pm%2)))))   //if it is initialize the lights interrupt and set wake up to 1
        {
            Lightsintrpt();
            wakeup = 1;
            speaker = 2;        //set speaker to 2 to turn it off
...
//custom functions to check the states of vairous inputs when in the set time state with no outputs
void setTimeConditions()
{
...
//custom functions to check the states of vairous inputs when in the set alarm state with no outputs
void setAlarmChanges()
{
    ...
    if(snoozedec == 1)              //if decrement was pressed based on what is blinking decrement hours or minutes
    {
        ...
        {
            Aminutes--;
            if(Aminutes == 255)     //if minutes = 255, 0 - 1 = 255, then reset minutes to 59
                {
                    Aminutes = 59;
                }
        }
